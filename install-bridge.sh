...
install_bridge() {
  ...
  ssh -o StrictHostKeyChecking=no jclouds@$SERVER 'sudo mv -f ~/haproxy-marathon-bridge /usr/local/bin/haproxy-marathon-bridge; sudo mv -f ~/refresh-haproxy /usr/local/bin/refresh-haproxy; sudo mv -f ~/haproxy.cron /etc/cron.d/haproxy.cron'
  echo "> DONE on $SERVER"
}
